...
public static class Mapper
{
    ...
    public static TDestination Map<TSource, TDestination>(TSource source, bool mergeOnly = false, int maxDepth = 10)
    {
        ...
        if (destObj is null) return default!;

        MapInternal(source, (TDestination)destObj,
            [], new Dictionary<object, object>(ReferenceEqualityComparer.Instance), mergeOnly, maxDepth);
        ...
        int maxDepth = 10)
    {
        if (Equals(source, null) ||
            Equals(destination, null)) return; // If either is null, skip mapping (per requirement)
    ...
    private static void MapInternal(object source, object destination, Dictionary<(object, Type), object> visited,
        Dictionary<object, object> referenceMap, bool mergeOnly = false, int maxDepth = 10, int currentDepth = 0)
    {
        ...
        // Existing mapping? (cycle) => short circuit
        if (referenceMap.ContainsKey(source))
            // Already mapped this source object; nothing further to do
            return;

        // Prevent circular reference infinite loops by tracking (source, destination_type) pairs
        if (!visited.TryAdd(visitedKey, destination)) return;
        referenceMap[source] = destination; // track source->destination reference
        ...
        foreach (var (srcProp, destProp) in pairs)
        {
            ...
            {
                MapEnumerable(srcValue, destination, destProp, visited, referenceMap, mergeOnly, maxDepth,
                    currentDepth);
                continue;
            }
            ...
            if (destCurrent == null)
            {
                ...
                if (destCurrent == null) continue; // Cannot construct destination nested object

                TrySetValue(destination, destProp, destCurrent);
            }

            MapInternal(srcValue, destCurrent, visited, referenceMap, mergeOnly, maxDepth, currentDepth + 1);
        }
    }

    private static void MapEnumerable(object srcValue, object destination, PropertyInfo destProp,
        Dictionary<(object, Type), object> visited, Dictionary<object, object> referenceMap, bool mergeOnly = false,
        int maxDepth = 10, int currentDepth = 0)
    {
        ...
        if (srcValue is not IEnumerable srcEnumerable) return;

        var destPropType = destProp.PropertyType;
        var (destIsArray, destElementType) = GetEnumerableElementType(destPropType);
        if (destElementType == null) return; // Unknown element type, skip
        ...
        foreach (var srcItem in srcEnumerable)
        {
            ...
            {
                if (!TryConvertValue(srcItem, destElementType, out var mapped)) continue;
            ...
            else
            {
                ...
                MapInternal(srcItem, newItem, visited, referenceMap, mergeOnly, maxDepth, currentDepth + 1);
                mappedItem = newItem;
            }
    ...
    private static void CopyListIntoCollection(object sourceList, object destinationCollection, Type elementType)
    {
        ...
        if (iCollectionType.IsInstanceOfType(destinationCollection))
        {
            ...
            foreach (var item in enumerator) addMethod!.Invoke(destinationCollection, [item]);
        ...
        {
            foreach (var item in (IEnumerable)sourceList) nonGenericList.Add(item);
    ...
    private static List<PropertyPair> BuildTypeMap(Type sourceType, Type destType)
    {
        ...
        foreach (var sp in sourceProps) sourceMap[sp.Name] = sp;

        var destMap = new Dictionary<string, PropertyInfo>(StringComparer.OrdinalIgnoreCase);
        foreach (var dp in destProps) destMap[dp.Name] = dp;
        ...
        try
        {
            ...
            else if (targetType == typeof(DateTimeOffset))
            {
                if (value is string s && DateTimeOffset.TryParse(s, CultureInfo.InvariantCulture,
                        DateTimeStyles.RoundtripKind, out var dataTimeOffset))
                {
                    converted = dataTimeOffset;
                    return true;
                }
            }
    ...
    private static object? CreateCollectionInstance(Type collectionType, Type elementType)
    {
        ...
        else
        {
            ...
            try
            {
                var instance = Activator.CreateInstance(collectionType, true);
    ...
    /// <summary>
    ///     Projects an IQueryable of TSource into IQueryable of TDestination by building a simple member-init expression
    ...
    public static IQueryable<TDestination> ProjectTo<TSource, TDestination>(this IQueryable<TSource> query)
    {
        ...
        var destProps = destType.GetProperties(BindingFlags.Public | BindingFlags.Instance)
            .Where(p => p.CanWrite && p.GetIndexParameters().Length == 0).ToList();

        var param = Expression.Parameter(sourceType, "src");
        var bindings = new List<MemberBinding>();
        foreach (var dp in destProps)
        {
            if (!sourceProps.TryGetValue(dp.Name, out var sp)) continue;
            // Only map simple types to keep expression server-translatable & performant
            if (!IsSimpleType(dp.PropertyType) && dp.PropertyType != sp.PropertyType) continue;
            if (!dp.PropertyType.IsAssignableFrom(sp.PropertyType)) continue;
            var access = Expression.Property(param, sp);
            bindings.Add(Expression.Bind(dp, access));
        }
